...
import { Button, Col, Container, Image, Modal, Row, Form, Card, Spinner } from "react-bootstrap";
import FormField from "../../components/layout/FormField";
import { MotionDiv } from "../../components";
import {  useLocation, useNavigate,useParams } from "react-router-dom";
import ModalTemplate from "../../components/modals/ModalTemplate";
import { useCreateProfMutation, useGetDomainsMutation, useGetProfByIdMutation, useGetSubdomainsMutation, useUpdateProfByIdMutation } from "../../features/apiSlice";
...
function AddEditProf() {
  ...
  const [updateProfById,{isLoading:updateLoading}] = useUpdateProfByIdMutation();
  ...
  useEffect(()=>{
     if(selectedSubdomains){
      ...
      const updatedsubdomains = subdomains.filter((item) => !selectedSubdomains.map(sub => sub?._id).includes(item?._id));
  ...
  const handleAddSubdomain = (e) => {
    ...
    setSelectedSubdomains(prev => [...prev, { sub_domain_name:label, _id:value }]);
  };

  const handleDeleteSubdomain = (item) => {
    const updatedSelectedDomain = selectedSubdomains.filter((s, i) => s?._id !== item?._id);
    setSelectedSubdomains(updatedSelectedDomain);
    setSubdomains([...subdomains,{sub_domain_name:item?.sub_domain_name,_id:item?._id}]);
...
setSelectedSubdomains(prof?.sub_domain)
    ...
    const handleSubmit = async(e)=>{
       try {
        ...
        const data = isEditProf? await updateProfById({id:id,data:{
            name: form?.profName,
            email: form?.email,
    ...
    <MotionDiv >
      ...
      <Row>
        ...
        {selectedSubdomains?.map((item,i)=>(
          <div className="domain-card p-2 my-1 d-flex" style={{ color: 'rgba(0, 0, 139, 1)' }} key={item._id}>
          <div className="row w-100">
              <div className="col">{item?.sub_domain_name}</div>
              <div className="col-auto"><FaRegRectangleXmark onClick={()=>handleDeleteSubdomain(item)} style={{cursor:'pointer'}} color="rgba(255, 22, 22, 1)"/></div>
          </div>
      </div>
...
{/* <input type="range"   value={form?.payPercentage}  id="volume" onChange={handleChange} name="payPercentage" min="0" max="100"/> */}
         ...
         <input type="number" className="border p-0 fw-bold text-center"  style={{color:'var(--primary-color)',width:'2.5rem'}} value={form?.payPercentage} onChange={handleChange} name="payPercentage" min="0" max="100"/><span style={{color:'var(--primary-color)'}}>%</span>
      ...
      <Row>
        <Col className="text-end">
          <Button variant="transparent" className="add-btn px-3 m-1" onClick={()=>navigate(-1)}>Cancel </Button>
          <Button className="blue-btn px-3 m-1" type="submit">{isEditProf?'Edit Prof': 'Add Prof'}</Button>
        </Col>
      </Row>
      </Form>
...
title={`Are you sure you want to  ${isEditProf?'update':'add'} this Prof. profile to user system`}
...
<ModalTemplate
  ...
  src={'/icons/tick.png'}
  title={`You have successfully ${isEditProf?'updated':'created'} new Prof. profile`}
