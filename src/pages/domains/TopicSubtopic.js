...
import { useGetSubTopicsMutation, useGetTopicsMutation } from '../../features/apiSlice'
    ...
    //   }
      ...
      useEffect(()=>{
        if(selectedTopic){
            ...
            fetchSubtopics({setSubtopics,id:selectedTopic,getSubtopics,params:`key=${subTopicQuery}`});
        }
  ...
  return (
   ...
   <Row className='my-3'>
        <Col className='text-end'>
            Exam/Certification By: <u style={{color:'rgba(128, 152, 249, 1)'}}>Edicexam.com</u>
        </Col>
    </Row>
   <Row>
    <Col>
        <Card className='p-3 border-0 shadow '>
             <p>Selected Area of Speciality: <span>Mechanical Engineering</span></p>
             <Row>

             <Col sm={7}><SearchField query={topicQuery} setQuery={setTopicQuery} placeholder='Search Topic'/></Col>
             <Col className='justify-content-end d-flex align-items-center'>        
             <Link to={`/admin/domains/all-topics/${id}`} style={{color:'rgba(0, 0, 155, 1)'}}><u>View All <FaChevronRight/></u></Link>
</Col>
             </Row>
...
<div className={`d-flex justify-content-between align-items-center border rounded p-2  ${selectedTopic === item?._id ?'selected-topic':''}`} onClick={()=>setSelectedTopic(item?._id)} style={{maxWidth:'500px',cursor:'pointer'}}>
    ...
    <Col>
    <Card className='p-3 border-0 shadow'>
             <p>Selected Topic: <span>Control Engineering</span></p>
             <Row>
             <Col sm={7}><SearchField placeholder='Search Subtopic' query={subTopicQuery} setQuery={setSubTopicQuery}/></Col>
             <Col className='justify-content-end d-flex align-items-center'>        
             <Link to={`/admin/domains/all-subtopics/${selectedTopic}`} style={{color:'rgba(0, 0, 155, 1)'}}><u>View All <FaChevronRight/></u></Link>
</Col>
             </Row>
            ...
            subtopics?.length >0 ? subtopics?.map((item,index)=>(
            <Col className='p-1' key={item?._id}>
<div className="d-flex justify-content-between align-items-center   p-2 " style={{borderLeft:'1px solid rgba(45, 50, 130, 1)',color:'rgba(45, 50, 130, 1)',maxWidth:'500px',cursor:'pointer'}} >
