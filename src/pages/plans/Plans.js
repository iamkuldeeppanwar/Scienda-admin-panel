...
export default function Plans() {
  ...
  useEffect(()=>{
     fetchSubdomains({setSubdomains,id:null,getSubdomains,params:query});
  },[])
  ...
  const column = [
    ...
    // "Domain Id",
    <>
    <span>Plans Available</span><br/>
    <span style={{color:'var(--main-blue)',fontSize:'0.9rem'}}>M | Q | Y</span>
    </>,
    "Actions"
  ];
  ...
  return (
    <MotionDiv>
      ...
      <Row>
          ...
          <CountCard title={'Total Amount Received'} count={2540}/>
          <CountCard title={'No. of Users'} count={50}/>
          ...
          {subdomains &&
            subdomains?.map((data, i) => (
              <tr key={data?._id} className="odd">
                ...
                <td>{ data?.professor?.length >0 ? data?.professor?.map(prof => prof.name).join(' | '): 'NA'}</td>
                {/* <td className="text-center">{data?.domainId}</td> */}
                <td className="text-center" style={{color:'rgba(97, 114, 243, 1)'}}>
  {
