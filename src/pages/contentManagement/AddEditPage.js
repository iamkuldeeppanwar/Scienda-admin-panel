...
import { useCreatePageMutation, useGetPageByIdMutation, useUpdatePageMutation } from "../../features/apiSlice";
...
const [updatePage] = useUpdatePageMutation()
  ...
  const submitHandler = async (e) => {
    ...
    try {
        ...
        setLoading(true)
        const data =  isEditPage? await updatePage({id,data:{title,description}}).unwrap() : await createPage({title,description}).unwrap();
        setLoading(false)
          ...
          <Row className="my-3">
            ...
            <Col className="text-end" md={11}>
              <Button className="blue-btn px-4" onClick={submitHandler}>
                {loading ? <Spinner size="sm" /> : isEditPage?'Update':'Add Page'}
              </Button>
            </Col>
          </Row>
        </Container>
